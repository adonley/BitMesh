...
public class PaymentChannelServerState {
    ...
    private Script multisigScript, redeemScript;
    ...
    PaymentChannelServerState(StoredServerChannel storedServerChannel, Wallet wallet, TransactionBroadcaster broadcaster) throws VerificationException {
        synchronized (storedServerChannel) {
            ...
            // this assumes that the multisig script is raw multisig
            // and it gets overwritten in provideRefundTransaction anyway
...
//        Script multisigPubKey = ScriptBuilder.createMultiSigOutputScript(2, ImmutableList.of(clientKey, serverKey));
        redeemScript = ScriptBuilder.createMultiSigOutputScript(2, ImmutableList.of(clientKey, serverKey));
...
//        TransactionSignature sig = refundTx.calculateSignature(0, serverKey, multisigPubKey, Transaction.SigHash.NONE, true);
        TransactionSignature sig = refundTx.calculateSignature(0, serverKey, redeemScript, Transaction.SigHash.NONE, true);
        /*
        log.info("Signed refund transaction with key: " + serverKey.toString());
        log.info("Signature is canonical: " + sig.isCanonical());
        log.info("r: " + sig.r.toString(10));
        log.info("s: " + sig.s.toString(10));
        log.info("sigtype: " + sig.sigHashMode().toString() + (sig.anyoneCanPay() ? " ANYONECANPAY" : ""));
        log.info("script: " + multisigPubKey.toString());
        log.info("script: " + new BigInteger(multisigPubKey.getProgram()).toString(16));
        List<ScriptChunk> scriptChunks = multisigPubKey.getChunks();
        String chunks = "";
        for (int i = 0 ; i < scriptChunks.size(); i++)
        {
            chunks += " " + scriptChunks.get(i).opcode;
        }
    ...
    public synchronized ListenableFuture<PaymentChannelServerState> provideMultiSigContract(final Transaction multisigContract) throws VerificationException {
        ...
        try {
            ...
            final Script expectedScript = ScriptBuilder.createP2SHOutputScript(2, Lists.newArrayList(clientKey, serverKey));
    ...
    public synchronized boolean incrementPayment(Coin refundSize, byte[] signatureBytes) throws VerificationException, ValueOutOfRangeException, InsufficientMoneyException {
        ...
        if (walletContract.getConfidence().getConfidenceType() == TransactionConfidence.ConfidenceType.DEAD) {
            close();
            // TODO: handle this by blacklisting ip address or something?
        ...
        Sha256Hash sighash = req.tx.hashForSignature(0, ScriptBuilder.createMultiSigOutputScript(2, Lists.newArrayList(clientKey, serverKey)), mode, true);
        //log.info("paymentTx: " + req.tx.getHashAsString());
        //log.info("checking payment signature against sighash: " + sighash.toBigInteger().toString(16));
        if (!clientKey.verify(sighash, signature) /*&&
            !clientKey.verify(sighash2, signature)*/)
...
//        TransactionSignature signature = tx.calculateSignature(0, serverKey, multisigScript, hashType, anyoneCanPay);
        TransactionSignature signature = tx.calculateSignature(0, serverKey, redeemScript, hashType, anyoneCanPay);
...
//        Script scriptSig = ScriptBuilder.createMultiSigInputScriptBytes(ImmutableList.of(bestValueSignature, mySig));
        TransactionSignature bestValueSig = TransactionSignature.decodeFromBitcoin(bestValueSignature, true);
        Script scriptSig = ScriptBuilder.createP2SHMultiSigInputScript(Arrays.asList(bestValueSig, signature), redeemScript);
    ...
    public synchronized ListenableFuture<Transaction> close() throws InsufficientMoneyException {
        if (storedServerChannel != null) {
            ...
            log.info("myKey: " + temp.myKey.toString());
        ...
        try {
            ...
            // TODO: make this fee correctly
//            feePaidForPayment = req.tx.getFee();
            feePaidForPayment = Transaction.REFERENCE_DEFAULT_MIN_TX_FEE;
            bestValueToMe = bestValueToMe.subtract(feePaidForPayment);
            log.info("Calculated fee is {}", feePaidForPayment);
            if (feePaidForPayment.compareTo(bestValueToMe) >= 0) {
                final String msg = String.format("Had to pay more in fees (%s) than the channel was worth (%s). Not closing channel.",
                        feePaidForPayment, bestValueToMe);
                log.warn(msg);
                return SettableFuture.create();
//                throw new InsufficientMoneyException(feePaidForPayment.subtract(bestValueToMe), msg);
            }
            // add output to ourselves
            tx.addOutput(bestValueToMe, wallet.getChangeAddress());
...
//            throw e;  // Don't fall through.
    ...
    public synchronized void storeChannelInWallet(@Nullable PaymentChannelServer connectedHandler) {
        ...
        StoredPaymentChannelServerStates channels =
                (StoredPaymentChannelServerStates) wallet.addOrGetExistingExtension(new StoredPaymentChannelServerStates(wallet, broadcaster));
