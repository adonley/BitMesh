...
public class PaymentChannelServer {
    ...
    public final int SERVER_MINOR_VERSION = 2;
    public final int APP_MINOR_VERSION     = 0;
    public final int BROWSER_MICROPAYMENTS_MINOR_VERSION = 1;
    public final int BROWSER_MACROPAYMENT_MINOR_VERSION = 2;
    public final boolean SEND_PAYMENT_ACKS = true;
    ...
    public Address fundingAddress;
    public ConnectedClient client;
    public Transaction finalTransaction;

    private int numFundingSent = 0;

    private Protos.ClientVersion clientVersion;
    ...
    protected final ReentrantLock channelLock = Threading.lock("payment-channel");
    ...
    public PaymentChannelServer(TransactionBroadcaster broadcaster, Wallet wallet,
        ...
        this.params = BitmeshServer.getInstance().getConfig().params;
        this.myKey = new ECKey();
        this.myAddress = myKey.toAddress(params);
        this.wallet.freshReceiveKey();
        //log.info("myKey: {}", (new BigInteger(1, myKey.getPubKey()).toString(16)));
    }
    ...
    private void receiveVersionMessage(Protos.TwoWayChannelMessage msg) throws VerificationException {
        ...
        Protos.TwoWayChannelMessage.Builder twoWayBuilder = Protos.TwoWayChannelMessage.newBuilder();
        twoWayBuilder.setType(Protos.TwoWayChannelMessage.MessageType.SERVER_VERSION);
        twoWayBuilder.setServerVersion(versionNegotiationBuilder);
        Protos.TwoWayChannelMessage message = twoWayBuilder.build();
        ...
        if (reopenChannelContractHash != null && reopenChannelContractHash.size() == 32) {
            ...
            if (channels != null) {
                ...
                if (storedServerChannel != null) {
                    ...
                    client.setReloadedChannel(true);
                    return;
                } else {
        ...
        long price = client.getConfig().getMaxPrice();
        log.info("Got initial version message. Responding with VERSIONS and PRICE: ", price);
        Protos.Price.Builder priceBuilder = Protos.Price.newBuilder()
                .setProposedPrice(price)
                .setMultisigKey(ByteString.copyFrom(myKey.getPubKey()));

        conn.sendToClient(Protos.TwoWayChannelMessage.newBuilder()
                .setType(Protos.TwoWayChannelMessage.MessageType.PRICE)
                .setPrice(priceBuilder)
                .build());

        // TODO: in future we will start a bargaining conversation here
        if (clientVersion.getMajor() == 1)
        {
            if (clientVersion.getMinor() == APP_MINOR_VERSION)
            {
            ...
            // else if micropayment browser just wait for a funding address
            else if (clientVersion.getMinor() == BROWSER_MICROPAYMENTS_MINOR_VERSION)
            {
                step = InitStep.WAITING_ON_FUNDING_ADDRESS;
            }
            // else if macropayment browser
            else if (clientVersion.getMinor() == BROWSER_MACROPAYMENT_MINOR_VERSION)
            {
                step = InitStep.WAITING_ON_FUNDING;
                BitmeshServer.getInstance().watchAddress(myAddress, this);
            }
        }

    }

    private void receiveFundingAddress(Protos.TwoWayChannelMessage msg) throws AddressFormatException
    {
        // TODO: in future we will end bargaining here
        client.getConfig().setNegotiatedPrice(client.getConfig().getMaxPrice());

        checkState(step == InitStep.WAITING_ON_FUNDING_ADDRESS && msg.hasFundingAddress());
        step = InitStep.WAITING_ON_UNSIGNED_REFUND;
        fundingAddress = new Address(BitmeshConfiguration.params, msg.getFundingAddress().getFundingAddress());
        log.info("Received funding address:" + fundingAddress.toString());
        BitmeshServer server = BitmeshServer.getInstance();
        server.watchAddress(fundingAddress, this);

        // in case for some reason they've already been funded by the time this is called
        // send any funding we've heard about
        // this can happen, for instance, if downloading dependencies takes too long and the websocket times out
        Set<Transaction> funding = BitmeshServer.getInstance().getFundingTransactionsForAddress(fundingAddress);
        for (Transaction tx : funding)
        {
//            if (!server.alreadySentTxs.contains(tx.getHashAsString()))
//            {
            log.info("Sending already seen tx to buyer: {}", tx.getHashAsString());
            sendFunding(tx);
//            }
        }
    }

    public void sendFunding(@Nonnull Transaction tx)
    {
        if (clientVersion.getMinor() == BROWSER_MACROPAYMENT_MINOR_VERSION)
        {
            long amountSpent = getAmountSentToAddress(tx, myAddress);
            totalSent += amountSpent;
            Protos.PaymentAck.Builder ackBuilder = Protos.PaymentAck.newBuilder()
                                                        .setAmountSpent(amountSpent)
                                                        .setAmountPurchased(amountSpent / client.getConfig().getMaxPrice());
            conn.sendToClient(Protos.TwoWayChannelMessage.newBuilder().setType(Protos.TwoWayChannelMessage.MessageType.PAYMENT_ACK)
                                                                        .setPaymentAck(ackBuilder).build());
            conn.paymentIncrease(Coin.valueOf(amountSpent), Coin.valueOf(totalSent), null);

        }
        else if (clientVersion.getMinor() == BROWSER_MICROPAYMENTS_MINOR_VERSION)
        {
            log.info("Sending funding tx {}", tx.getHashAsString());
            Protos.Funding.Builder fundingBuilder = Protos.Funding.newBuilder().setTransaction(ByteString.copyFrom(
                    tx.bitcoinSerialize()));
/*        Protos.Funding.Builder fundingBuilder = Protos.Funding.newBuilder()
                .setSatoshis(output.getValue().value)
                .setOutputIndex(output.getIndex())
                .setFundingScript(ByteString.copyFrom(output.getScriptBytes()))
                .setFundingTxHashBytes(ByteString.copyFrom(output.getParentTransaction().getHashAsString().getBytes()))
                .setRefundAddress("n3VLc94EyRue5ponkpeirV8dLgbEbothbY"); */

            conn.sendToClient(Protos.TwoWayChannelMessage.newBuilder()
                    .setType(Protos.TwoWayChannelMessage.MessageType.FUNDING)
                    .setFunding(fundingBuilder)
                    .build());
            numFundingSent++;
            BitmeshServer.getInstance().alreadySentTxs.add(tx.getHashAsString());
...
//        checkState(step == InitStep.WAITING_ON_UNSIGNED_REFUND && msg.hasProvideRefund());
        checkState(msg.hasProvideRefund());
        ...
        log.info("Sending signature: " + new BigInteger(signature).toString(16));
    ...
    private void multisigContractPropogated(Protos.ProvideContract providedContract, Sha256Hash contractHash) {
        ...
        try {
            ...
            try {
                receiveUpdatePaymentMessage(providedContract.getInitialPayment(), SEND_PAYMENT_ACKS);
    ...
    private void receiveUpdatePaymentMessage(Protos.UpdatePayment msg, boolean sendAck) throws VerificationException, ValueOutOfRangeException, InsufficientMoneyException {
        ...
        {
            log.warn("Received payment update without increase in payment.");
        }
    ...
    public void receiveMessage(Protos.TwoWayChannelMessage msg) {
        ...
        try {
            ...
            try {
                ...
                log.error("Caught invalid address exception from client", e);
                error(e.getMessage(), Protos.Error.ErrorCode.INVALID_ADDRESS, PaymentChannelCloseException.CloseReason.REMOTE_SENT_INVALID_MESSAGE);
            } catch (ValueOutOfRangeException e) {
    ...
    private void settlePayment(final PaymentChannelCloseException.CloseReason clientRequestedClose) throws InsufficientMoneyException {
        ...
        Futures.addCallback(state.close(), new FutureCallback<Transaction>() {
            ...
            public void onSuccess(Transaction result) {
                // instead of sending an immediate CLOSE, we wait for the connected client to do that after time is up
                // if time is up before the final transaction propagates, then we simply omit the transaction from the
                // close message
     ...
     * {@link ServerConnection#destroyConnection(network.bitmesh.channels.PaymentChannelCloseException.CloseReason)} or
     ...
     * {@link ServerConnection#destroyConnection(network.bitmesh.channels.PaymentChannelCloseException.CloseReason)}. Note that this does not broadcast
        ...
        try {
            if (connectionOpen /*&& !channelSettling*/) {
                ...
                if (finalTransaction != null)
                {
                    // Result can be null on various error paths, like if we never actually opened
                    // properly and so on.
                    msg.getSettlementBuilder().setTx(ByteString.copyFrom(finalTransaction.bitcoinSerialize()));
                    log.info("Sending CLOSE back with broadcast settlement tx.");
                }
                else
                {
                    log.info("Sending CLOSE back without settlement tx.");
                }
                conn.sendToClient(msg.build());
    ...
    public long getAmountSentToAddress(Transaction tx, Address address) {
        long sum = 0;
        ...
        {
            if (output.getAddressFromP2PKHScript(params).equals(address))
            {
    ...
    public void stopWatchingAddress() {
        if (clientVersion.getMinor() == BROWSER_MICROPAYMENTS_MINOR_VERSION)
        {
            BitmeshServer.getInstance().stopWatchingAddress(fundingAddress, this);
        }
        else if (clientVersion.getMinor() == BROWSER_MACROPAYMENT_MINOR_VERSION)
        {
            BitmeshServer.getInstance().stopWatchingAddress(myAddress, this);
        }
    }
}
