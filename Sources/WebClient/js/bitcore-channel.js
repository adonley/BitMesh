...
var Output = require('bitcore').Transaction.Output;
var Input = require('bitcore').Transaction.Input;
//var RawMultisigInput = require('bitcore').RawMultisigInput;
var _ = require('bitcore').deps._;
var Signature = require('bitcore').crypto.Signature;
...
Consumer.prototype.processFunding = function(utxo) {
  ...
  this.commitmentTx.outputs = [];
  this.commitmentTx.addOutput(new Output({
//    script: new Script(this.commitmentTx.address),
    script: this.commitmentTx.outscript,
    satoshis: this.commitmentTx.inputAmount - this.commitmentTx.getFee()
  }));
};
...
//  $.checkState(this.commitmentTx.outputs[0].script.isMultisigOut());
  ...
  this.refundTx = new Refund().from(multisigOut, this.commitmentTx.publicKeys, 2);
/*
  this.refundTx.addInput(new RawMultisigInput({
    script: this.commitmentTx.outscript,
    prevTxId: this.commitmentTx.id,
    outputIndex: 0,
    output: multisigOut
  }));
*/
...
//  this.refundTx.nLockTime = this.expires;
  this.refundTx.nLockTime = Math.round(new Date().getTime() / 1000) + ONE_DAY;
 ...
 */
Consumer.prototype.validateRefund = function(refund, providerSignature) {
...
//  this.incrementPaymentBy()
//  this.paymentTx.sign(this.commitmentKey, Signature.SIGHASH_ANYONECANPAY | Signature.SIGHASH_SINGLE);
  return true;
};
 ...
 */
Consumer.prototype.incrementPaymentBy = function(satoshis, sigtype) {
  this.paymentTx.updateValue(satoshis);
  this.paymentTx.sign(this.commitmentKey, sigtype);
  console.log("incrementing payment by " + satoshis + " to " + this.paymentTx.paid);
...
function Commitment(opts) {
  ...
  this.outscript = Script.buildScriptHashOut(Script.buildMultisigOut(this.publicKeys, 2, { noSorting: true}));
  this.address = this.outscript.toAddress();
//  this.outscript = Script.buildMultisigOut(this.publicKeys, 2, { noSorting: true});
...
//  We send the correct change amount to ourselves and do SIGHASH_SINGLE
  var refund = this.amount - this.paid;
  if (refund !== 0)
  {
    this._addOutput(new Output({
      script: new Script(new Address(this.changeAddress)),
      satoshis: refund
    }));
  }

//  this.to(this.changeAddress, this.amount - this.paid);
